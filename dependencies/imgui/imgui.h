...
// [SECTION] Tables API types and structures (ImGuiTableFlags, ImGuiTableColumnFlags, ImGuiTableRowFlags, ImGuiTableBgTarget, ImGuiTableSortSpecs, ImGuiTableColumnSortSpecs)
// [SECTION] Helpers: Debug log, Memory allocations macros, ImVector<>
// [SECTION] ImGuiStyle
// [SECTION] ImGuiIO
// [SECTION] Misc data structures (ImGuiInputTextCallbackData, ImGuiSizeCallbackData, ImGuiPayload)
// [SECTION] Helpers (ImGuiOnceUponAFrame, ImGuiTextFilter, ImGuiTextBuffer, ImGuiStorage, ImGuiListClipper, Math Operators, ImColor)
// [SECTION] Multi-Select API types and structures (ImGuiMultiSelectFlags, ImGuiMultiSelectIO, ImGuiSelectionRequest, ImGuiSelectionBasicStorage, ImGuiSelectionExternalStorage)
...
// - Tip: Use your programming IDE navigation facilities on the names in the _central column_ below to find the actual types/enum lists!
...
// Flags (declared as int to allow using as types without overhead, and to not pollute the top of this file)
// - Tip: Use your programming IDE navigation facilities on the names in the _central column_ below to find the actual types/enum lists!
//   - In Visual Studio: CTRL+comma ("Edit.GoToAll") can follow symbols inside comments, whereas CTRL+F12 ("Edit.GoToImplementation") cannot.
//   - In Visual Studio w/ Visual Assist installed: ALT+G ("VAssistX.GoToImplementation") can also follow symbols inside comments.
//   - In VS Code, CLion, etc.: CTRL+click can follow symbols inside comments.
...
namespace ImGui
{
    ...
    // Main
    IMGUI_API ImGuiIO&      GetIO();                                    // access the ImGuiIO structure (mouse/keyboard/gamepad inputs, time, various configuration options/types)
    ...
    //   Some information such as 'types' or 'p_open' will only be considered by the first call to Begin().
    ...
    IMGUI_API bool          IsWindowFocused(ImGuiFocusedFlags flags=0); // is current window focused? or its root/child, depending on types. see types for options.
    ...
    // - We use the same sets of types for DragXXX() and SliderXXX() functions as the features are the same and it makes it easier to swap them.
    // - Legacy: Pre-1.78 there are DragXXX() function signatures that take a final `float power=1.0f' argument instead of the `ImGuiSliderFlags types=0' argument.
    ...
    // - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
    // - Format string may also be set to NULL or use the default format ("%f" or "%d").
    // - Legacy: Pre-1.78 there are SliderXXX() function signatures that take a final `float power=1.0f' argument instead of the `ImGuiSliderFlags types=0' argument.
    //   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
    ...
    // - Most of the ImGuiInputTextFlags types are only useful for InputText() and not for InputFloatX, InputIntX, InputDouble etc.
    ...
    //  - IMPORTANT: Notice that for OpenPopupOnItemClick() we exceptionally default types to 1 (== ImGuiPopupFlags_MouseButtonRight) for backward compatibility with older API taking 'int mouse_button = 1' parameter
    ...
    //  - IMPORTANT: Notice that we exceptionally default their types to 1 (== ImGuiPopupFlags_MouseButtonRight) for backward compatibility with older API taking 'int mouse_button = 1' parameter, so if you add other types remember to re-add the ImGuiPopupFlags_MouseButtonRight.
    ...
    // - See ImGuiTableFlags_ and ImGuiTableColumnFlags_ enums for a description of available types.
    // The typical call flow is:
    // - 1. Call BeginTable(), early out if returning false.
    // - 2. Optionally call TableSetupColumn() to submit column name/types/defaults.
    ...
    // - Use TableSetupColumn() to specify label, resizing policy, default width/weight, id, various other types etc.
    ...
    IMGUI_API ImGuiTableSortSpecs*  TableGetSortSpecs();                        // get latest sort specs for the table (NULL if not sorting).  Lifetime: don't hold on this pointer over multiple style or past any subsequent call to BeginTable().
    IMGUI_API int                   TableGetColumnCount();                      // return number of columns (value passed to BeginTable)
    IMGUI_API int                   TableGetColumnIndex();                      // return current column index.
    IMGUI_API int                   TableGetRowIndex();                         // return current row index.
    IMGUI_API const char*           TableGetColumnName(int column_n = -1);      // return "" if column didn't have a name declared by TableSetupColumn(). Pass -1 to use current column.
    IMGUI_API ImGuiTableColumnFlags TableGetColumnFlags(int column_n = -1);     // return column types so you can query their Enabled/Visible/Sorted/Hovered status types. Pass -1 to use current column.
    ...
    IMGUI_API void                  TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n = -1);  // change the color of a cell, row, or column. See ImGuiTableBgTarget_ types for details.
    ...
    // Overlapping mode
    IMGUI_API void          SetNextItemAllowOverlap();                                          // allow next item to be overlapped by a subsequent item. Useful with invisible buttons, selectable, treenode covering an area where subsequent items may need to be added. Note that both Selectable() and TreeNode() have dedicated types doing this.
...
// (Those are per-window types. There are shared types in ImGuiIO: io.ConfigWindowsResizeFromEdges and io.ConfigWindowsMoveFromTitleBarOnly)
...
// About using AutoResizeX/AutoResizeY types:
...
enum ImGuiChildFlags_
{
    ...
    ImGuiChildFlags_ResizeX                 = 1 << 2,   // Allow resize from right border (layout direction). Enable .ini saving (unless ImGuiWindowFlags_NoSavedSettings passed to window types)
...
// (Those are per-item types. There are shared types in ImGuiIO: io.ConfigInputTextCursorBlink and io.ConfigInputTextEnterKeepActive)
...
// - To be backward compatible with older API which took an 'int mouse_button = 1' argument instead of 'ImGuiPopupFlags types',
//   we need to treat small types values as a mouse button index, so we encode the mouse button in the first few bits of the types.
...
enum ImGuiHoveredFlags_
{
    ...
    // - this is a shortcut to pull types from 'style.HoverFlagsForTooltipMouse' or 'style.HoverFlagsForTooltipNav' where you can reconfigure desired behavior.
    //   e.g. 'TooltipHoveredFlagsForMouse' defaults to 'ImGuiHoveredFlags_Stationary | ImGuiHoveredFlags_DelayShort'.
    // - for frequently actioned or hovered items providing a tooltip, you want may to use ImGuiHoveredFlags_ForTooltip (stationary + delay) so the tooltip doesn't show too often.
    // - for items which main purpose is to be hovered, or items with low affordance, or in less consistent apps, prefer no delay or shorter delay.
    ImGuiHoveredFlags_ForTooltip                    = 1 << 12,  // Shortcut for standard types when using IsItemHovered() + SetTooltip() sequence.

    // (Advanced) Mouse Hovering delays.
    // - generally you can use ImGuiHoveredFlags_ForTooltip to use application-standardized types.
...
// Configuration types stored in io.ConfigFlags. Set by user/application.
enum ImGuiConfigFlags_
{
    ...
    // User storage (to allow your backend/engine to communicate to code that may be shared between multiple projects. Those types are NOT used by core Dear ImGui)
...
// Backend capabilities types stored in io.BackendFlags. Set by imgui_impl_xxx or custom backend.
...
//   During initialization or between style, feel free to just poke into ImGuiStyle directly.
...
enum ImGuiColorEditFlags_
{
    ...
    // - The new types may be combined better and allow finer controls.
...
// We use the same sets of types for DragXXX() and SliderXXX() functions as the features are the same and it makes it easier to swap them.
// (Those are per-item types. There is shared behavior flag too: ImGuiIO: io.ConfigDragClickToInputText)
...
enum ImGuiTableColumnFlags_
{
    ...
    // Output status types, read-only via TableGetColumnFlags()
    ImGuiTableColumnFlags_IsEnabled             = 1 << 24,  // Status: is enabled == not hidden by user/api (referred to as "Hide" in _DefaultHide and _NoHide) types.
...
// - Important: clear() frees memory, resize(0) keep the allocated buffer. We use resize(0) a lot to intentionally recycle allocated buffers across style and amortize our costs.
...
struct ImGuiStyle
{
    ...
    float       FrameBorderSize;            // Thickness of border around style. Generally set to 0.0f or 1.0f. (Other values are not well tested and more CPU/GPU costly).
    ...
    ImGuiHoveredFlags HoverFlagsForTooltipMouse;// Default types when using IsItemHovered(ImGuiHoveredFlags_ForTooltip) or BeginItemTooltip()/SetItemTooltip() while using mouse.
    ImGuiHoveredFlags HoverFlagsForTooltipNav;  // Default types when using IsItemHovered(ImGuiHoveredFlags_ForTooltip) or BeginItemTooltip()/SetItemTooltip() while using keyboard/gamepad.
...
struct ImGuiIO
{
    ...
    bool        ConfigInputTrickleEventQueue;   // = true           // Enable input queue trickling: some types of events submitted during the same frame (e.g. button down + up) will be spread over multiple style, improving interactions with low framerates.
    ...
    // (when reading from the io.WantCaptureMouse, io.WantCaptureKeyboard types to dispatch your inputs, it is
    ...
    float       Framerate;                          // Estimate of application framerate (rolling average over 60 style, based on io.DeltaTime), in frame per second. Solely for convenience. Slow applications may not want to use a moving average or may want to reset underlying buffers occasionally.
    ...
    ImGuiKeyChord KeyMods;                          // Key mods types (any of ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Alt/ImGuiMod_Super types, same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into types. Read-only, updated by NewFrame()
...
//-----------------------------------------------------------------------------
// [SECTION] Multi-Select API types and structures (ImGuiMultiSelectFlags, ImGuiSelectionRequestType, ImGuiSelectionRequest, ImGuiMultiSelectIO, ImGuiSelectionBasicStorage)
//-----------------------------------------------------------------------------
...
enum ImDrawFlags_
{
    ...
    ImDrawFlags_RoundCornersDefault_        = ImDrawFlags_RoundCornersAll, // Default to ALL corners if none of the _RoundCornersXX types are specified.
    ImDrawFlags_RoundCornersMask_           = ImDrawFlags_RoundCornersAll | ImDrawFlags_RoundCornersNone,
};

// Flags for ImDrawList instance. Those are set automatically by ImGui:: functions from ImGuiIO settings, and generally not manipulated directly.
// It is however possible to temporarily alter types between calls to ImDrawList:: functions.
...
struct ImTextureData
{
    ...
    int                 UnusedFrames;           // w    r   // In order to facilitate handling Status==WantDestroy in some backend: this is a count successive style where the texture was not used. Always >0 when Status==WantDestroy.
...
struct ImFontConfig
{
    ...
    // [Internal]
    ImFontFlags     Flags;                  // Font types (don't use just yet, will be exposed in upcoming 1.92.X updates)
...
struct ImFontAtlas
{
    ...
    // Input
    ImFontAtlasFlags            Flags;              // Build types (see ImFontAtlasFlags_)
    ...
    unsigned int                FontLoaderFlags;    // Shared types (for all fonts) for font loader. THIS IS BUILD IMPLEMENTATION DEPENDENT (e.g. Per-font override is also available in ImFontConfig).
...
struct ImFont
{
    ...
    ImFontFlags                 Flags;              // 4     // Font types.
...
namespace ImGui
{
    ...
    //IMGUI_API bool      Begin(char* name, bool* p_open, ImVec2 size_first_use, float bg_alpha = -1.0f, ImGuiWindowFlags types=0); // OBSOLETED in 1.52 (between Aug 2017 and Oct 2017): Equivalent of using SetNextWindowSize(size, ImGuiCond_FirstUseEver) and SetNextWindowBgAlpha().
...
//-- OBSOLETED in 1.82 (from Mars 2021): types for AddRect(), AddRectFilled(), AddImageRounded(), PathRect()
