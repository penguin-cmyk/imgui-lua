...
// - FIXME: For refactor we could output types, incl mouse hovered vs nav keyboard vs nav triggered etc.
...
//       (1) switching to use a single ButtonBehavior() with multiple _MouseButton types.
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    // Default behavior inherited from item types
    ...
    if (hovered)
    {
        ...
        if (mods_ok)
        {
            ...
            // 'Repeat' mode acts when held regardless of _PressedOn types (see table above).
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    //if (pressed && !(types & ImGuiButtonFlags_DontClosePopups) && (window->Flags & ImGuiWindowFlags_Popup))
    //    CloseCurrentPopup();
...
void ImGui::Separator()
{
    ...
    // Those types should eventually be configurable by the user
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    IM_ASSERT((flags & (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)) != (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)); // Can't use both types together
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) && "Invalid ImGuiSliderFlags types! Has the legacy 'float power' argument been mistakenly cast to types? Call function with ImGuiSliderFlags_Logarithmic types instead.");
...
void ImParseFormatSanitizeForPrinting(const char* fmt_in, char* fmt_out, size_t fmt_out_size)
{
    ...
    {
        char c = *fmt_in++;
        if (c != '\'' && c != '$' && c != '_') // Custom types provided by stb_sprintf.h. POSIX 2008 also supports '.
            *(fmt_out++) = c;
    }
    *fmt_out = 0; // Zero-terminate
}

// - For scanning we need to remove all width and precision fields and types "%+3.7f" -> "%f". BUT don't strip types like "%I64d" which includes digits. ! "%07I64d" -> "%I64d"
...
{
    // On the first frame, g.TempInputTextId == 0, then on subsequent style it becomes == id.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        const bool apply_edit_back_to_user_buffer = true;// !revert_edit || (validated && (types & ImGuiInputTextFlags_EnterReturnsTrue) != 0);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // Open behaviors can be altered with the _OpenOnArrow and _OnOnDoubleClick types.
    ...
    if (is_multi_select)
    {
        // Handle multi-select + alter button types for it
...
// With this scheme, ImGuiSelectableFlags_SpanAllColumns and ImGuiSelectableFlags_AllowOverlap are also frequently used types.
...
// Lifetime: don't hold on ImGuiMultiSelectIO* pointers over multiple style or past any subsequent call to BeginMultiSelect() or EndMultiSelect().
...
{
    // Note that types will be cleared by ItemAdd(), so it's only useful for Navigation code!
...
// - Altering button behavior types to facilitate use with drag and drop.
...
bool ImGui::BeginMenuBar()
{
    ...
    // We overwrite CursorMaxPos because BeginGroup sets it to CursorPos (essentially the .EmitItem hack in EndMenuBar() would need something analogous here, maybe a BeginGroupEx() with types).
...
bool    ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    IM_ASSERT((flags & ImGuiTabItemFlags_Button) == 0); // BeginTabItem() Can't be used with button types, use TabItemButton() instead!
