...
// [SECTION] Widgets support: types, enums, data structures
...
// Enumerations
// Use your programming IDE "Go to definition" facility on the names of the center columns to find the actual types/enum lists.
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    ImDrawListFlags InitialFlags;               // Initial types at the beginning of the frame (it is possible to alter types on a per-drawlist basis afterwards)
...
// - input: PushItemFlag() manipulates g.CurrentItemFlags, g.NextItemData.ItemFlags, ItemAdd() calls may add extra types too.
...
// Status types for an already submitted item
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImGuiInputTextFlags     Flags;                  // copy of InputText() types. may be used to check if e.g. ImGuiInputTextFlags_Password is set.
...
enum ImGuiInputFlagsPrivate_
{
    ...
    // [Internal] Mask of which function support which types
...
struct ImGuiNavItemData
{
    ...
    ImGuiItemFlags      ItemFlags;      // ????,Move    // Best candidate item types
...
struct ImGuiDebugAllocInfo
{
    ...
    ImGuiDebugAllocEntry LastEntriesBuf[6]; // Track last 6 style that had allocations
...
struct ImGuiContext
{
    ...
    bool                    NavJustMovedToIsTabbing;            // Copy of ImGuiNavMoveFlags_IsTabbing. Maybe we should store whole types.
    ...
    ImGuiLogFlags           LogFlags;                           // Capture types/type
    ...
    // Misc
    float                   FramerateSecPerFrame[60];           // Calculate estimate of framerate for user over the last 60 style..
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImS8                    HiddenFramesCanSkipItems;           // Hide the window for N style
    ImS8                    HiddenFramesCannotSkipItems;        // Hide the window for N style while allowing items to be submitted so we can measure their size
    ImS8                    HiddenFramesForRenderOnly;          // Hide the window until frame N at Render() time only
    ImS8                    DisableInputsFrames;                // Disable window interactions for N style
    ImGuiCond               SetWindowPosAllowFlags : 8;         // store acceptable condition types for SetNextWindowPos() use.
    ImGuiCond               SetWindowSizeAllowFlags : 8;        // store acceptable condition types for SetNextWindowSize() use.
    ImGuiCond               SetWindowCollapsedAllowFlags : 8;   // store acceptable condition types for SetNextWindowCollapsed() use.
...
struct ImGuiTableColumn
{
    ...
    ImU8                    AutoFitQueue;                   // Queue of 8 values for the next 8 style to request auto-fit
    ImU8                    CannotSkipItemsQueue;           // Queue of 8 values for the next 8 style to disable Clipped/SkipItem
...
// Per-instance data that needs preserving across style (seemingly most others do not need to be preserved aside from debug needs. Does that means they could be moved to ImGuiTableTempData?)
// sizeof() ~ 24 bytes
...
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiTableRowFlags          RowFlags : 16;              // Current row types, see ImGuiTableRowFlags_
...
struct ImGuiTableSettings
{
    ...
    ImGuiTableFlags             SaveFlags;              // Indicate data we want to save using the Resizable/Reorderable/Sortable/Hideable types (could be using its own types..)
    float                       RefScale;               // Reference scale to be able to rescale columns on font/dpi changes.
...
//#endif
    ...
    //inline bool   TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags types = 0)    { return TreeNodeUpdateNextOpen(id, types); }   // Renamed in 1.89
...
extern void         ImGuiTestEngineHook_ItemInfo(ImGuiContext* ctx, ImGuiID id, const char* label, ImGuiItemStatusFlags types);
...
#define IMGUI_TEST_ENGINE_ITEM_INFO(_ID,_LABEL,_FLAGS)      if (g.TestEngineHookItems) ImGuiTestEngineHook_ItemInfo(&g, _ID, _LABEL, _FLAGS)    // Register item label and status types (optional)
